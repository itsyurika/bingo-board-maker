...
describe('Controls', () => {
  ...
  describe('file upload', () => {
    it('should handle file selection', async () => {
      ...
      const input = screen.getByRole('button', { name: /upload json file/i });
  ...
  describe('header customization', () => {
    ...
    it('should call onHeaderTextChange when title changes', async () => {
      ...
      const calls = mockProps.onHeaderTextChange.mock.calls;
      const lastCall = calls[calls.length - 1];
      expect(lastCall).toEqual(['title', 'Test TitleN']);
    });
    ...
    it('should call onHeaderTextChange when instructions change', async () => {
      ...
      await user.type(instructionsInput, 'I');

      // Check that the function was called with the final value
      const calls = mockProps.onHeaderTextChange.mock.calls;
      const lastCall = calls[calls.length - 1];
      expect(lastCall).toEqual(['instructions', 'Test InstructionsI']);
    });
    ...
    it('should call onHeaderTextChange when subtitle changes', async () => {
      ...
      await user.type(subtitleInput, 'S');

      // Check that the function was called with the final value
      const calls = mockProps.onHeaderTextChange.mock.calls;
      const lastCall = calls[calls.length - 1];
      expect(lastCall).toEqual(['subtitle', 'Test SubtitleS']);
    });
