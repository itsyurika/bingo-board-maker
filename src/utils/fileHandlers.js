...
import { validateFile, validateAndParseJSON, validatePromptsData } from './validation'
 ...
 */
export async function handleFileUpload(file, includeFreeSpace, { setLoadingState, showError }) {
  // Check rate limiting
  if (!uploadRateLimiter.canAttempt()) {
    const remainingTime = Math.ceil(uploadRateLimiter.getRemainingTime() / 1000)
    throw new Error(`Too many upload attempts. Please wait ${remainingTime} seconds before trying again.`)
  }
  ...
  try {
    ...
    const totalPrompts = Object.values(sanitizedData).reduce((sum, prompts) => sum + prompts.length, 0)
    const additionalWarnings = checkPerformanceWarnings(file.size, totalPrompts)
...
export function validateFile(file) {
  ...
  if (!file.name.toLowerCase().endsWith(FILE_CONSTRAINTS.ALLOWED_EXTENSION)) return false
  if (file.size > FILE_CONSTRAINTS.MAX_SIZE) return false
  return true
}