...
describe('boardGenerator', () => {
  ...
  describe('generateBoard', () => {
    it('should generate a board with 25 tiles including free space by default', () => {
      ...
      const freeSpaceCount = board.filter(
        (tile) => tile === 'FREE_SPACE'
      ).length;
    ...
    it('should generate a board with 25 prompts when includeFreeSpace is false', () => {
      ...
      const promptCount = board.filter(
        (tile) => tile !== 'FREE_SPACE' && tile !== ''
      ).length;
    ...
    it('should handle categories with different sizes', () => {
      const unevenPrompts = {
        ...
        large: Array.from({ length: 22 }, (_, i) => `large_prompt_${i + 1}`),
      };
    ...
    describe('error handling', () => {
      it('should throw error for invalid input (not an object)', () => {
        expect(() => generateBoard(null)).toThrow(
          'Invalid JSON: Must be an object with categories'
        );
        expect(() => generateBoard('invalid')).toThrow(
          'Invalid JSON: Must be an object with categories'
        );
        expect(() => generateBoard(123)).toThrow(
          'Invalid JSON: Must be an object with categories'
        );
      });

      it('should throw error for empty categories object', () => {
        expect(() => generateBoard({})).toThrow(
          'Invalid JSON: Must contain at least one category'
        );
      });
      ...
      it('should throw error for non-array category values', () => {
        ...
        expect(() => generateBoard(invalidPrompts)).toThrow(
          'Invalid JSON: Category "personal" must be an array'
        );
      });
      ...
      it('should throw error for empty category arrays', () => {
        ...
        expect(() => generateBoard(invalidPrompts)).toThrow(
          'Invalid JSON: Category "personal" cannot be empty'
        );
      });
      ...
      it('should throw error for insufficient total prompts', () => {
        ...
        expect(() => generateBoard(insufficientPrompts)).toThrow(
          'Insufficient prompts: Need at least 24 prompts total, found 2'
        );
      });
    });
    ...
    describe('edge cases', () => {
      ...
      it('should handle more prompts than needed', () => {
        const manyPrompts = {
          category1: Array.from(
            { length: 30 },
            (_, i) => `cat1_prompt_${i + 1}`
          ),
          category2: Array.from(
            { length: 25 },
            (_, i) => `cat2_prompt_${i + 1}`
          ),
        };
  ...
  describe('generateBoardFromFile', () => {
    it('should successfully generate board from valid JSON file with free space', async () => {
      const mockFile = {
        text: vi.fn().mockResolvedValue(JSON.stringify(mockPrompts)),
      };
    ...
    it('should throw error for invalid JSON syntax', async () => {
      ...
      await expect(generateBoardFromFile(mockFile)).rejects.toThrow(
        'Invalid JSON file: Please check file format and try again'
      );
    });
    ...
    it('should throw validation error for invalid prompts structure', async () => {
      ...
      const mockFile = {
        text: vi.fn().mockResolvedValue(JSON.stringify(invalidPrompts)),
      };
  ...
  describe('distribution logic', () => {
    ...
    it('should handle remainder distribution correctly', () => {
      const testPrompts = {
        cat1: Array.from({ length: 12 }, (_, i) => `cat1_${i}`),
        cat2: Array.from({ length: 12 }, (_, i) => `cat2_${i}`),
      };
